"""This module implements the rome toolchain rule.
"""

RomeInfo = provider(
    doc = "Information about how to invoke the rome executable.",
    fields = {
        "rome_binary": "Path to the rome executable for the target platform.",
        "tool_files": """Files required in runfiles to make the rome executable available.

May be empty if the target_tool_path points to a locally installed rome binary.""",
    },
)
...
def _rome_toolchain_impl(ctx):
    ...
    template_variables = platform_common.TemplateVariableInfo({
        "ROME_BINARY_PATH": target_tool_path,
    })
    default = DefaultInfo(
